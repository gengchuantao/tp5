...
class IncomeController extends IndexController
{
    public function index(){
        ...
        $receipt_id = Request::instance()->get('receipt_id');
        ...
        Session::set('receipt_id',$receipt_id);
...
");
    ...
    function export(){
        ...
        $data  = Db::name('income')
            ...
            ->select();
        //设置表头：
        $head = [ 'ID','生产工号', '合同号', '款项类型', '欠款类型', '入金分公司', '收款人', '收款编号', '收款款项名称', '分类', '收款拆分金额', '来款方式', '是否买断合同', '买方单位', '大客户', '大项目', '跨区域', '入金-事业部', '事业部所在区域', '拆款日期', '审核状态', '事业部收入', '季度薪酬包增加', '季度费用包增加', '年终奖','条件'];
        //数据中对应的字段，用于读取相应数据：
        $keys = [ 'id','product_id', 'contract_id', 'income_type', 'arrears_type', 'company', 'payee', 'receipt_id', 'fund_name', 'classification', 'split_amount', 'income_way', 'if_buyoff', 'buyer', 'big_customer', 'big_project', 'cross_region', 'income_bu', 'branch', 'income_date', 'audit_status', 'bu_income', 'quarter_pay_add', 'quarter_cost_add', 'year_pay_add', 'condition'];
