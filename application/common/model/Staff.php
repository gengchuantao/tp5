...
// 我的类名叫做Staff，对应的文件名为Staff.php，该类继承了Model类，Model我们在文件头中，提前使用use进行了导入。
class Staff extends Model
{
    protected $table = 'helc_staff';
    /**
     * 用户登录
     ...
     */
    static public function login($staff_id, $password)
    {
        // 验证用户是否存在
        $map = array('staff_id' => $staff_id);
        $Staff = self::get($map);
        ...
        if (!is_null($Staff)) {
            ...
            if ($Staff->checkPassword($password)) {
                // 登录
                session('staff_id', $Staff->getData('id'));
                return true;
            }
        }
        return false;
