...
$.ajax({
    type:"post",
    url:"/../tp5/public/index.php/index/bufinance/getYearBalanceByBuName",
    dataType:'json',
    async:true,
    success:function(data){
        for(let i=0;i<data.length;i++){
            $("#bu_accrued_amount").val(data[i].year_balance);
        }
    },
    error: function (xhr) {
        /*错误信息处理*/
...
$.ajax({
    type:"post",
    url:"/../tp5/public/index.php/index/bufinance/getConditionByBuName",
    dataType:'json',
    async:true,
    success:function(data){
        for(let i=0;i<data.length;i++){
            $("#bu_condition").val(data[i].condition);
        }
    },
    error: function (xhr) {
        /*错误信息处理*/
...
function AddNew(){
    ...
    let bu_condition=document.getElementById('bu_condition').value;
    if(!type){
        alert('请选择类别！');
        return ;
    }
